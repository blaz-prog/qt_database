...
class Drzava(Base):
    ...
    def get_number_of_matches(filter_text):
        ...
        with Session(engine) as select_session:
            stmt = text("""
            SELECT count(*) FROM drzava WHERE naziv ilike :filter
            """)
    ...
    def get_matches(filter_text):
        ...
        '''
        with Session(engine) as select_session:
            stmt = select(Drzava).where(Drzava.naziv.ilike(f"{filter_text}%"))
    ...
    def get_exact_match(filter_text):
        with Session(engine) as select_session:
            stmt = select(Drzava).where(Drzava.naziv == filter_text)
